Module.register("MMM-PublicTransit", {
  ...
  start() {
    this.busSchedule = [
      { route_short_name: "Loading...", departure_time: Date.now()/1000 + 60, trip_headsign: "" }
    ];
  ...
  getDom() {
    ...
    busTimesContainer.style.flexGrow = '1'; 
    ...
    while (i < this.busSchedule.length && j < this.config.displayed_entries) {
      ...
      routeInfo.style.color = 'white'; 
      routeInfo.style.display = 'flex'; 
      routeInfo.style.justifyContent = 'space-between'; 
      ...
      if (this.config.showTime) {
          ...
          let timeStr = dateObj.toLocaleTimeString('en-US', { hour: 'numeric', minute: '2-digit', hour12: true }).toLowerCase();
    ...
    if (this.config.showlogo) {
      ...
      transitlogoContainer.style.justifyContent = this.config.logoLocation; 
  ...
  activeHours() {
    ...
    if (startHour === undefined || stopHour === undefined || activeDays === undefined) return true;
